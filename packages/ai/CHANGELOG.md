...
- Fixed strict tool schemas being sent with validation keywords that are not accepted across provider strict subsets. `minLength`, `maxLength`, `pattern`, `format`, `minimum`, `maximum`, `multipleOf`, `minItems`, `maxItems` and `uniqueItems` are now stripped from the wire schema, so a tool stays strict on every provider instead of risking an admission-time rejection on the strictest ones. The trade is deliberate: the model no longer sees those constraints, which remain enforced post-hoc by original-schema validation and the reprompt loop.
- Fixed tools with reused sub-schemas dropping out of constrained sampling. Local, acyclic `$ref`/`$defs` are emitted by TypeBox for any reused sub-schema and are now strictified rather than rejected, including each entry of the definition pool. External references, dangling pointers and recursive reference graphs remain unstrictifiable, because a provider's grammar compiler only sees the document it is sent. `oneOf` is rewritten to the equivalent `anyOf`, and object branches inside a union are strictified instead of disqualifying the tool.
...
- Fixed the object-root requirement being applied where providers do not impose it. It now covers tool parameters in every dialect, since a root union has no `properties` to build a grammar from and Anthropic's `input_schema` must be an object, plus OpenAI-family structured output. Anthropic structured output is exempt: its own transformer handles a root union and `output_config` accepts one, so rejecting it merely sent the raw, untransformed union with open object branches.
- Fixed the inbound half of the strict-mode nullable-optional bargain not resolving references, and not judging union arms independently. `validateToolArguments` now resolves local `$ref` pointers before deciding whether a `null` is a strict-mode placeholder or a value the schema permits, and walks `allOf`/`anyOf`/`oneOf` branches. Guessing structurally discarded legitimate nulls for properties whose definition is nullable; skipping `$ref` properties left placeholder nulls in place and failed the call; and aggregating `required` across mutually exclusive union arms preserved a placeholder whenever another arm required the same property, after which the surviving `null` was coerced into a fabricated value. Unresolvable references, including external URLs and cycles, preserve the null.
